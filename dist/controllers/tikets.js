...
exports.router.get("/check", async (req, res) => {
    ...
    const found = rows[0]; // เอาผลลัพธ์แรก
...
exports.router.post("/buy-number", async (req, res) => {
    ...
    if (!Number.isInteger(drawId) ||
        ...
        const tx = await db_1.conn.promise().getConnection(); // เริ่มต้นการเชื่อมต่อฐานข้อมูล
        try {
            ...
            if (!draw) {
               ...
               VALUES (?, ?, ?, 'SOLD', ?, NOW())`, [realDrawId, number, price, userId]);
                ...
                const [upd] = await tx.execute(`UPDATE wallets SET balance = balance - ? WHERE id=? AND balance >= ?`, [price, wallet.id, price]);
