...
const checkPrizes = async (req, res) => {
    ...
    const ticketNumber = typeof req.query.ticketNumber === "string"
        ...
        : undefined;
    const buyerId = req.query.buyerUserId ?? req.query.buyer_user_id
    ...
    if (!Number.isInteger(drawNumber) ||
        ...
        res.status(400).json({
            success: false,
            message: "ต้องระบุ drawNumber:int และอย่างน้อยหนึ่งใน ticketNumber:string หรือ buyerUserId:int",
        });
        return;
    }
    // ดึงข้อมูลงวดจาก draw_number
    const [drawRows] = await db_1.conn.promise().query(`SELECT id, status,
    ...
    const draw = Array.isArray(drawRows) && drawRows[0];
    if (!draw) {
        res
            .status(404)
...
// Route ใหม่
exports.router.get("/prize-check", asyncHandler(checkPrizes));
const claimPrize = async (req, res) => {
    const drawNumber = Number(req.body.drawNumber ?? req.body.draw_number);
    const ticketNumber = typeof req.body.ticketNumber === "string"
        ? req.body.ticketNumber.trim()
        : undefined;
    const buyerId = req.body.buyerUserId ?? req.body.buyer_user_id
    ...
    try {
        ...
        // 1) โหลด draw
        const [drawRows] = await tx.query(`SELECT id, status,
              win1_full, win2_full, win3_full,
              win_last3, win_last2,
              prize1_amount, prize2_amount, prize3_amount,
              last3_amount, last2_amount
       ...
       WHERE draw_number = ?`, [drawNumber]);
        ...
        // 2) lock ticket
        const [tRows] = await tx.query(`SELECT id, draw_id, ticket_number, status, buyer_user_id
       FROM tickets
       WHERE draw_id = ? AND ticket_number = ?
       FOR UPDATE`, [draw.id, ticketNumber]);
        const ticket = Array.isArray(tRows) && tRows[0];
        if (!ticket) {
            await tx.rollback();
            res.status(404).json({
                success: false,
        ...
        const [wRows] = await tx.query(`SELECT id, user_id, balance
     FROM wallets
    WHERE user_id = ?
    FOR UPDATE`, [buyerId]);
        const wallet = Array.isArray(wRows) && wRows[0];
        if (!wallet) {
            await tx.rollback();
            res.status(409).json({
                success: false,
        ...
        try {
            await tx.query(`INSERT INTO wallet_transactions
           (wallet_id, tx_type, amount, ref_type, ref_id, note)
         VALUES
           (?, 'PRIZE', ?, 'TICKET', ?, ?)`, [wallet.id, judge.awardedAmount, ticket.id, note]);
        }
        catch (e) {
        ...
        // 8) อัปเดตยอดกระเป๋า
        await tx.query(`UPDATE wallets
         SET balance = balance + ?
       WHERE id = ?`, [judge.awardedAmount, wallet.id]);
        // 9) เปลี่ยนสถานะตั๋วเป็น REDEEMED
        await tx.query(`UPDATE tickets
         SET status = 'REDEEMED'
       WHERE id = ?`, [ticket.id]);
        await tx.commit();
        res.json({
...
// route
exports.router.post("/claim", asyncHandler(claimPrize));
...
exports.router.get("/bydate", async (req, res) => {
    try {
        const { date, drawNumber } = req.query;
        // ===== validate =====
        if (!date) {
            res
                .status(400)
...
exports.router.get("/", (req, res) => {
    const sql = `
      ...
      LIMIT 1
    `;
    db_1.conn.execute(sql, [], (err, results) => {
        if (err) {
...
exports.router.post("/randomlotto", async (req, res) => {
    // ===== รับค่า (เหมือนเดิม) =====
    ...
    // ฟังก์ชันสุ่มเลข
    const randomDigits = (len) => Math.floor(Math.random() * Math.pow(10, len))
        .toString()
        .padStart(len, "0");
    // helper function: สุ่ม k รายการแบบไม่ซ้ำจากอาร์เรย์
    const pickUnique = (arr, k) => {
    ...
    try {
        ...
        const [openRows] = (await tx.query(`SELECT id, draw_number
         FROM draws
        WHERE status='OPEN'
        ...
        if (!open) {
            const [lastRows] = (await tx.query(`SELECT id, draw_number
           FROM draws
          ORDER BY draw_number DESC
          LIMIT 1
          FOR UPDATE`));
            const nextNumber = lastRows.length > 0 ? lastRows[0].draw_number + 1 : 1;
            const [insOpen] = (await tx.execute(`INSERT INTO draws (
        ...
        let prize2;
        let prize3;
        let last3; // <-- จะเซ็ตหลังจากรู้ prize1 แล้ว
        let last2;
        if (sourceMode === "SOLD_ONLY") {
            // ถ้าโหมดคือ 'SOLD_ONLY' ให้เลือกเลขจากตั๋วที่ขายไปแล้ว
            const [soldRows] = (await tx.query(`SELECT ticket_number
       FROM tickets
      ...
      FOR UPDATE`, [open.id]));
        ...
        const [openIns] = (await tx.execute(`INSERT INTO draws (
         draw_number, draw_date, status, source_mode,
         prize1_amount, prize2_amount, prize3_amount, last3_amount, last2_amount,
         created_at
       ) VALUES (
        ...
        const [oneRows] = (await tx.query(`SELECT
         id,
         draw_number AS drawNumber,
         DATE_FORMAT(draw_date, '%Y-%m-%d') AS drawDate,
         status,
